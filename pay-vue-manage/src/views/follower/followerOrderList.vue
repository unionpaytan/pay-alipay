...
<template>
  <div>
   ...
   <div class="flex flex-m tc topbox searchBox">
      <div class="flex-1 bor">
        <div>总交易金额<span style="font-size:12px;">(元)</span></div>
 ...
 <div style="margin-left: 1px" class="flex flex-m mr20">
        <el-button
          ...
          type="primary">搜索 (ENTER回车键或ESC键)</el-button>
      </div>
    ...
    >
      ...
      <el-table-column align="center" prop label="收款人" >
        <template slot-scope="scope">
          <span> {{ scope.row.payerName }} <br /></span>
          <!-- <span>{{ scope.row.payerIdentity }}</span>  -->
        </template>
      </el-table-column>

 <el-table-column align="center" prop="coinType" label="收款码类别" width="100">
         <template slot-scope="scope">
          {{ scope.row.coinType | coinTypeFilter}}
        </template>
     </el-table-column>
     
     <el-table-column align="center" prop label="收款码ID" width="185">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" prop="status" label="收款状态"  width="100">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" prop="status" label="支付状态"  width="100">
        <template slot-scope="scope">
          <span :style="scope.row.status == 2 ? 'color:#ff0000' : (scope.row.status == 1 ? 'color:#67C23A' : '')">{{ scope.row.status | depositStatusFilter}}</span> 
        </template>
      </el-table-column> 
        ...
        <template slot-scope="scope">  
            <div>
             ...
             <div>
              <el-button
                style="color: #67c23a" 
                v-if="scope.row.status == '0' && scope.row.preStatus == '0'"
 ...
 //获取实时成功交易总金额
  ...
  /**
    ...
    handel_withdreawStatus_manual_showup(row, type) {
      ...
      this.forceTitle = type == 1 ? "人工确认收款" : "人工设为失败";
    ...
    handel_withdreawStatus_success(row) {
      ...
      let userData = (sessionStorage.userData && JSON.parse(sessionStorage.userData)) || {};
      let doubleConfirm = userData.doubleConfirm; //是否二次确认
      console.log("doubleConfirm - ",doubleConfirm);
      this.merchant_data.withdrawNo = row.withdrawNo;
      this.forceTitle = "确认收款"
      this.merchant_data.amt = row.amt;
      this.merchant_data.createTime = row.createTime;
      this.merchant_data.merchantId =userData.merchantId;
      this.merchant_data.nowTime = moment(new Date()).format(
    ...
    handel_withdreawStatus_force(data){ 
       this.m_loading = true;
       this.loadingStatus = true;
       let userData = (sessionStorage.userData && JSON.parse(sessionStorage.userData)) || {};
       let doubleConfirm = userData.doubleConfirm; //是否二次确认
       console.log("doubleConfirm - ",doubleConfirm);
       if (doubleConfirm == "0") {
        this.m_api.setDepositStatusSuccess({
          ...
          this.m_api.setDepositPreStatusSuccess({
            googleCode:data.googleCode,
            withdrawNo: data.withdrawNo,
            tableMerchantId: userData.merchantId,
          })
