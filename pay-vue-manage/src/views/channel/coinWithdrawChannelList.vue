<!-- 三方代付通道列表  coinWithdrawChannelList-->
<template>
  <div>
    ...
    <div class="flex flex-m mb20 searchBox">
      <div class="flex flex-m mr20">
        <span class="w120">代付通道编号:</span>
        <el-input
          size="medium"
          class="w180"
          v-model="search_data.channelCode"
          clearable
          placeholder="三方代付通道编号"
        ></el-input>
      </div>
      <div class="flex flex-m mr20">
        <span class="w120">代付通道名称:</span>
        <el-input
          size="medium"
          class="w180"
          v-model="search_data.channelName"
          clearable
          placeholder="三方代付通道名称"
        ></el-input>
      </div>
      <div class="flex flex-m mr20">
        <span class="w120">代付通道状态:</span>
    ...
    <!-- prop 数据库里的字段 -->
      ...
      <el-table-column align="center" prop="channelSingle" label="单笔手续费(USDT/笔)" width="160"></el-table-column>
    
      <el-table-column align="center" prop="coinNum" label="钱包数量"></el-table-column>
      <!--  状态-->
      <el-table-column align="center" label="状态"  width="110" >
        <template slot-scope="scope">
          <span :class="scope.row.status =='1'?'green':'red'">{{scope.row.status | coinStatus}}</span>
        </template>
      </el-table-column>
...
<div>
    ...
    >
      <el-form :model="formData" :rules="m_rules" ref="ruleForm">
        <el-form-item class="flex-1" label="通道名称" label-width="100px" clearable prop="channelName">
           <!-- :disabled="!m_isadd" -->
          <el-input 
            placeholder="代付通道名称"
            clearable
        ...
        <el-form-item class="flex-1" label="下发费率 %" label-width="100px" clearable prop="channelRate">
          <el-input
            placeholder="商户下发汇率若为0.5%,填:0.5"
        ...
        <el-form-item class="flex-1" label="单笔手续费" label-width="100px" clearable prop="channelSingle">
          <el-input 
        ...
        <el-form-item class="flex-1" label="通道标识" label-width="100px" clearable prop="remark">
          <el-input placeholder="通道标识" v-model="formData.remark" clearable auto-complete="off"></el-input>
        </el-form-item>

        <el-form-item class="flex-1" label="联系人" label-width="100px" clearable prop="contact">
          <el-input placeholder="联系人" v-model="formData.contact" clearable auto-complete="off"></el-input>
        </el-form-item>

        <el-form-item class="flex-1" label="邮箱" label-width="100px" clearable prop="email">
          <el-input placeholder="邮箱" v-model="formData.email" clearable auto-complete="off"></el-input>
        </el-form-item>

        <el-form-item class="flex-1" label="手机号" label-width="100px" prop="phone">
          <el-input
            placeholder="手机号"
            type="number"
        ...
        <el-form :model="balance_data" :rules="balanceForm_rules" ref="balanceForm">
          <div class="flex flex-m">
            <el-form-item class="flex-1" label="通道编号" label-width="120px">
              <span>{{balance_data.channelCode}}</span>
            </el-form-item>
          </div>

          <div class="flex flex-m">
            <el-form-item class="flex-1" label="通道名称" label-width="120px">
              <span>{{balance_data.channelName}}</span>
            </el-form-item>
          </div>

          <div class="flex flex-m">
            <el-form-item class="flex-1" label="手续费余额" label-width="120px">
              <span class="red">{{balance_data.amt}} USDT</span>
            </el-form-item>
          </div>

          <el-form-item label="充值金额" label-width="120px" prop="amt">
            <div class="flex flex-m">
              <el-input
                @mousewheel.native.prevent
                type="number"
                ...
                auto-complete="off"
              ></el-input>
              <div class="red ml15 fs18 w200">{{balance_data.rechargeAmt + ' USDT'}}</div>
            </div>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getDatalist() {
      ...
      this.m_api
        .queryCoinWithdrawChannelList(data)
        .then(res => {
    ...
    addData() {
      this.m_loading = true;
      this.m_api.registerCoinWithdrawChannel(this.formData)
        .then(res => {
          this.m_page.page = 1;
    ...
    mer_addForm() { 
      let channelName = this.formData.channelName != null ? this.formData.channelName : "";
    ...
    handel_modifyChannelStatus(row) {
      ...
      this.m_confirm(msgData).then(res => {
        if (res) {
          this.modifyCoinWithdrawChannelStatus(data);
        }
      });
    },
   ...
   gettingHighestAmtBychannelCode(v){
      //异步操作
      this.m_api.gettingHighestAmtBychannelCode({channelCode:v}).then(res => {
    ...
    batchModifyCoinWithdrawChannelInfoStatus(status) {
      ...
      this.m_confirm({ msg: `是否确 批量 ${txt} 通道` }).then(res => {
        if (res) {
          this.m_api.batchModifyCoinWithdrawChannelInfoStatus(form).then(res => {
            this.m_success("操作成功");
            this.getDatalist();
          });
        }
